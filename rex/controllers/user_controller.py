...
def get_totp_uri(otp_secret, user):
  return 'otpauth://totp/MacKayShields:{0}?secret={1}&issuer=MacKayShields' \
...
def settings():
    ...
    return render_template('account/setting.html', data=data)
...
def updatewallet():
    ...
    if request.method == 'POST':
        ...
        if request.form['wallet'] == '':
            flash({'msg':'Invalid wallet', 'type':'error'})
            return redirect('/user/setting')
        
        if check_password(user.password_transaction, request.form['password']) == False:
                flash({'msg':'Invalid password. Please try again!', 'type':'error'})
                return redirect('/user/setting')
...
def update_password():
    ...
    if request.method == 'POST':
        ...
        if check_password(user.password, old_password) == False:
            flash({'msg':'Invalid old password. Please try again!', 'type':'error'})
            return redirect('/user/setting')
        if new_password != repeat_new_password:
            flash({'msg':'Password repeat incorrectly. Please try again!', 'type':'error'})
            return redirect('/user/setting')
...
def send_mail_update_wallet(email,link_active):
    html = """
      ...
      Dear """+str(email)+""",<br><br></span> 
       ...
       <p>Please click on the link below to update your wallet address</p>  
       <br/>
...
def update_wallet_address():
    ...
    if request.method == 'POST':
        ...
        if int(check_send) < 60:
            ...
            check_address_eth = re.match('^0x[a-fA-F0-9]{40}$', ethereum)
            
            if bitcoin == "" or check_address_btc == False:
                flash({'msg':'Please enter your bitcoin wallet address.', 'type':'error'})
                return redirect('/user/my-profile')
...
def update_wallet_address_active(code):
    ...
    if get_wallet is not None:
        db.users.update({ "customer_id" : get_wallet['uid'] }, { '$set': { 
            "wallet.ethereum.address": get_wallet['ethereum'] ,
            "wallet.bitcoin.address": get_wallet['bitcoin'] 
        }})
        db.updatewallets.update({'code' : code},{ '$set': { 
            "status": 1 
        }})
        flash({'msg':'Update the wallet address successfully', 'type':'success'})
    return redirect('/user/my-profile')
    ...
    if int(positon) == 1 or int(positon) == 2:
        if int(positon) == 1:
            if binary.left == '':
                ...
                flash({'msg':'Invalid Position. Please try again!', 'type':'error'})
                return redirect('/account/dashboard')
        if int(positon) == 2:
            if binary.right == '':
                return render_template('account/signup_in_tree.html', data=value)
...
def registersubmit_intree():
    ...
    if request.method == 'POST':
        ...
        #     flash({'msg':'Invalid Telephone', 'type':'error'})
        ...
        if check_username is not None and check_email is not None and check_phone is not None:
            flash({'msg':'Invalid Username', 'type':'error'})
            flash({'msg':'Invalid Email', 'type':'error'})
            flash({'msg':'Invalid Telephone', 'type':'error'})
            return redirect('/user/new/'+str(request.form['position'])+'/'+str(request.form['p_binary'])+'/'+str(request.form['ref'])+'')
        ...
        if new_wallet == '':
            flash({'msg':'Invalid Wallet BTC', 'type':'error'})
            return redirect('/user/new/'+str(request.form['position'])+'/'+str(request.form['p_binary'])+'/'+str(request.form['ref'])+'')
        ...
        if recaptcha == '':
            flash({'msg':'Please check Captcha', 'type':'error'})
            return redirect('/user/new/'+str(request.form['position'])+'/'+str(request.form['p_binary'])+'/'+str(request.form['ref'])+'')
        ...
        if response['success'] == 1 and int(request.form['position']) == 1 or int(request.form['position']) == 2:
            ...
            flash({'msg':'Wrong captcha', 'type':'error'})
            return redirect('/user/register/%s'%(request.form['ref']))
...
def confirm_activecode(customer_id):
    if request.method == 'POST':
      ...
      if user is not None:
        ...
        flash({'msg':'Incorrect code', 'type':'error'})
